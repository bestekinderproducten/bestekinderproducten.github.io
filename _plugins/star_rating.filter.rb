module StarRatingFilter
	...
	Star_imageTag = "<img src=\"#{Star_imagesLoc}/%s\" class=\"icon icon-star\" alt=\"%s\" />"
	...
	def star_rating(rating)
		...
		wholeStars.times do
			htmlOutput += Star_imageTag % ["icon-star.svg", "#{ratingAltText}"]
			ratingAltText = ""
		end

		if (halfStar == 1)
			htmlOutput += Star_imageTag % ["star_half.svg", "#{ratingAltText}"]
			ratingAltText = ""
		end

		clearStars.times do
			htmlOutput += Star_imageTag % ["icon-star-empty.svg", "#{ratingAltText}"]
			ratingAltText = ""
		end
